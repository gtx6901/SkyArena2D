...
def test_fireable_cache_drops_target_killed_by_delayed_resolution() -> None:
    cfg = EnvConfig()
    cfg.teams.red_fighters = 1
    cfg.teams.blue_fighters = 1
    cfg.spawn.jitter = 0.0
    cfg.weapon.attack_effect_delay = 1
    cfg.weapon.hit_prob_enable = False
    cfg.weapon.long_range = 500.0
    cfg.radar.fighter_range = 500.0
    cfg.dynamics.default_fighter_speed = 0.0
    ...
    assert not bool(state.cache.red_fireable_long[0, 0])
    assert not bool(state.cache.red_fireable_short[0, 0])
